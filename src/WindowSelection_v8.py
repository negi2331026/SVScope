...
def readsCLIP3(CLIPRecord, ovlcutoff=0.5, mapQcutoff=5, lengthThreshold=100000):
    ...
    if len(chrom_pair)-1 not in BPSite:
        BPList.append(chrom[-1] +":" + str(refstart[-1]) +"_"+ chrom[-1]+ ":" +str(refstart[0])+"|%s|SoloBP|%s" % (readID, mapQ[-1]))
...
def FetchSpanReadDf(bedFile, chrom, start,end, cutoff=5):
    ...
    DatPool = []
    for bedF in bedFile.split(","):
        aimBed = pysam.TabixFile(bedF)
        ReadList = pd.DataFrame([FetchAimRegion(x, start,end) for x in aimBed.fetch(chrom,start,end) if (int(x.split("\t")[4])>=cutoff)], columns=['readID', 'refstart', 'refend', 'readstart', 'readend'])
...
def FindCandidateSVWindow(bedFileTumor, bedFileNormal, faiFile,
    ...
    os.system('sort -k1,1 -k2,2n -T ./ %s/tmpDEL.bed -o %s/tmpDEL.bed && bedtools merge -i %s/tmpDEL.bed -d 200 -c 4,4 -o count_distinct,distinct | awk \'$4>3 {print $0"\tDEL"}\' > %s/CandidateDEL.tumor.merged.bed && rm %s/tmpDEL.bed' % (savedir, savedir, savedir, savedir, savedir))
    resDf_INS_tumor.to_csv("%s/tmpINS.bed" % savedir, sep="\t", header=None, index=False)
    os.system('sort -k1,1 -k2,2n -T ./ %s/tmpINS.bed -o %s/tmpINS.bed && bedtools merge -i %s/tmpINS.bed -d 200 -c 4,4 -o count_distinct,distinct | awk \'$4>3 {print $0"\tINS"}\' > %s/CandidateINS.tumor.merged.bed' % (savedir, savedir, savedir, savedir))
    # Version 8 Update : Consider knwon tandem repeat window to avoid the loss of somatic tandem repeat insertions 
    os.system('sort -k1,1 -k2,2n -T ./ %s/tmpINS.bed -o %s/tmpINS.bed && bedtools intersect -b %s/tmpINS.bed -a %s -wa -wb> %s/CandidateLC.tumor.bed && rm %s/tmpINS.bed' % (savedir, savedir, savedir, LowComplex, savedir, savedir))
    ...
    Candidate_CLIP_Others_Result['SpanReadsT'] = Candidate_CLIP_Others_Result.apply(lambda x: FetchSpanReadDf(bedFileTumor, x['chrom'], np.max([x['start']-50, 0]), x['end']+50).index, axis=1)
    Candidate_CLIP_Others_Result['SpanReadsN'] = Candidate_CLIP_Others_Result.apply(lambda x: FetchSpanReadDf(bedFileNormal, x['chrom'], np.max([x['start']-50,0]), x['end']+50).index, axis=1)
    ...
    if Candidate_CLIP_Others_Result_GoodSpan.loc[Candidate_CLIP_Others_Result_GoodSpan['BPType']=='DEL'].shape[0] > 0:
        ...
        os.system('sort -k1,1 -k2,2n -T ./ %s/CandidateLargeDEL.tumor.merged.bed -o %s/CandidateLargeDEL.tumor.merged.bed' % (savedir, savedir))
    ...
    if Candidate_CLIP_Others_Result_GoodSpan.loc[Candidate_CLIP_Others_Result_GoodSpan['BPType']=='DUP'].shape[0] > 0:
        ...
        os.system('sort -k1,1 -k2,2n -T ./ %s/CandidateDUP.tumor.merged.bed -o %s/CandidateDUP.tumor.merged.bed' % (savedir, savedir))
    ...
    # Update V9 add sort -u cmd 
    # os.system('sort -u -T %s %s/CandidateSpan.tumor.merged.bed -o %s/CandidateSpan.tumor.merged.bed' % (savedir, savedir, savedir))
    ...
    # Update V9 add sort -u cmd 
    os.system('sort -u -T %s %s/CandidateSpan.tumor.merged.decision.somatic.bed -o %s/CandidateSpan.tumor.merged.decision.somatic.bed' % (savedir, savedir, savedir))
...
def main(args):
    ...
    for i,C in enumerate(chrom):
        chromDict[C] = np.sum(LenList[:i])
    spanSV,DEL,DUP,INV,TRA = FindCandidateSVWindow(args.bedFileTumor, args.bedFileNormal, faiFile=args.faiFile, LowComplex=args.tandemRepeatFile, savedir=args.savedir, cpu=int(args.thread))
